...
fn register_candidates(range: RangeInclusive<AccountId>) {
	...
	for ii in range {
		...
		assert_ok!(CollatorSelection::register_as_candidate(RuntimeOrigin::signed(ii),));
		System::assert_has_event(RuntimeEvent::CollatorSelection(Event::StakeAdded {
			staker: ii,
			candidate: ii,
			amount: 10,
		}));
		System::assert_last_event(RuntimeEvent::CollatorSelection(Event::CandidateAdded {
			account_id: ii,
			deposit: 10,
		}));
...
fn basic_setup_works() {
	new_test_ext().execute_with(|| {
		...
		#[cfg(feature = "try-runtime")]
		{
			...
			assert_ok!(
				<CollatorSelection as frame_support::traits::Hooks<BlockNumberFor<Test>>>::try_state(
					1
				)
			);
		}
...
fn it_should_set_invulnerables() {
	new_test_ext().execute_with(|| {
		...
		System::assert_last_event(RuntimeEvent::CollatorSelection(Event::NewInvulnerables {
...
fn cannot_empty_invulnerables_if_not_enough_candidates() {
	new_test_ext().execute_with(|| {
		assert_noop!(
			CollatorSelection::set_invulnerables(RuntimeOrigin::signed(RootAccount::get()), vec![]),
...
fn add_invulnerable_works() {
	new_test_ext().execute_with(|| {
		...
		System::assert_last_event(RuntimeEvent::CollatorSelection(Event::InvulnerableAdded {
			account_id: new,
		}));
...
fn remove_invulnerable_works() {
	new_test_ext().execute_with(|| {
		...
		System::assert_last_event(RuntimeEvent::CollatorSelection(Event::InvulnerableRemoved {
			account_id: 2,
		}));
...
fn candidate_to_invulnerable_works() {
	new_test_ext().execute_with(|| {
		...
		System::assert_has_event(RuntimeEvent::CollatorSelection(Event::CandidateRemoved {
			account_id: 3,
		}));
		System::assert_has_event(RuntimeEvent::CollatorSelection(Event::InvulnerableAdded {
			account_id: 3,
		}));
		System::assert_has_event(RuntimeEvent::CollatorSelection(Event::StakeRemoved {
			staker: 3,
			candidate: 3,
			amount: 10,
		}));
...
fn set_desired_candidates_works() {
	new_test_ext().execute_with(|| {
		...
		System::assert_last_event(RuntimeEvent::CollatorSelection(Event::NewDesiredCandidates {
...
fn set_candidacy_bond_empty_candidate_list() {
	new_test_ext().execute_with(|| {
		...
		System::assert_last_event(RuntimeEvent::CollatorSelection(Event::NewCandidacyBond {
			bond_amount: 7,
		}));
...
fn cannot_set_candidacy_bond_lower_than_min_stake() {
	new_test_ext().execute_with(|| {
		...
		// then
		assert_noop!(
			CollatorSelection::set_candidacy_bond(RuntimeOrigin::signed(RootAccount::get()), 1),
...
fn cannot_take_candidate_slot_if_list_not_full() {
	new_test_ext().execute_with(|| {
		...
		register_keys(22);
		assert_noop!(
			CollatorSelection::take_candidate_slot(RuntimeOrigin::signed(22), 50u64.into(), 3),
...
fn cannot_take_candidate_slot_if_target_invalid() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			CollatorSelection::take_candidate_slot(RuntimeOrigin::signed(3), 11u64.into(), 24),
...
fn cannot_take_candidate_slot_if_poor() {
	new_test_ext().execute_with(|| {
		...
		System::assert_last_event(RuntimeEvent::CollatorSelection(Event::CandidateReplaced {
			old: 4,
			new: 3,
			deposit: 20,
		}));
...
fn cannot_take_candidate_slot_if_insufficient_deposit() {
	new_test_ext().execute_with(|| {
		...
		register_candidates(3..=3);
		assert_ok!(CollatorSelection::stake(RuntimeOrigin::signed(3), 3, 60u64.into()));
...
fn cannot_take_candidate_slot_if_deposit_less_than_target() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			CollatorSelection::take_candidate_slot(RuntimeOrigin::signed(23), 20u64.into(), 3),
			Error::<Test>::InsufficientBond,
		);
...
fn candidate_list_works() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(CollatorSelection::stake(RuntimeOrigin::signed(5), 5, 20));
		assert_ok!(CollatorSelection::stake(RuntimeOrigin::signed(3), 3, 30));
...
fn session_management_increase_bid_with_list_update() {
	new_test_ext().execute_with(|| {
		...
		register_candidates(3..=5);
		assert_ok!(CollatorSelection::stake(RuntimeOrigin::signed(5), 5, 60));
...
fn session_management_reciprocal_outbidding() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(CollatorSelection::stake(RuntimeOrigin::signed(4), 4, 70));
		assert_ok!(CollatorSelection::stake(RuntimeOrigin::signed(3), 3, 70));
...
fn cannot_stake_if_not_candidate() {
	new_test_ext().execute_with(|| {
		// invulnerable
		assert_noop!(
			CollatorSelection::stake(RuntimeOrigin::signed(4), 1, 1),
			Error::<Test>::NotCandidate
		);
		// not registered as candidate
		assert_noop!(
			CollatorSelection::stake(RuntimeOrigin::signed(4), 5, 15),
			Error::<Test>::NotCandidate
		);
	});
}
...
fn cannot_stake_if_under_minstake() {
	new_test_ext().execute_with(|| {
		...
		register_candidates(3..=3);
		assert_noop!(
			CollatorSelection::stake(RuntimeOrigin::signed(4), 3, 1),
			Error::<Test>::InsufficientStake
		);
		assert_ok!(CollatorSelection::stake(RuntimeOrigin::signed(4), 3, 2));
		assert_eq!(Balances::free_balance(4), 98);
		assert_eq!(Stake::<Test>::get(3, 4), 2);

		// After adding MinStake it should work
		assert_ok!(CollatorSelection::stake(RuntimeOrigin::signed(4), 3, 1));
...
fn cannot_stake_too_many_candidates() {
	new_test_ext().execute_with(|| {
		...
		for i in 3..=18 {
			assert_ok!(CollatorSelection::stake(RuntimeOrigin::signed(1), i, 2));
		}
		assert_eq!(StakeCount::<Test>::get(1), 16);
		assert_noop!(
			CollatorSelection::stake(RuntimeOrigin::signed(1), 19, 2),
...
fn cannot_stake_invulnerable() {
	new_test_ext().execute_with(|| {
		assert_noop!(
			CollatorSelection::stake(RuntimeOrigin::signed(3), 1, 2),
			Error::<Test>::NotCandidate
		);
	});
}
...
fn unstake_from_candidate() {
	new_test_ext().execute_with(|| {
		...
		System::assert_last_event(RuntimeEvent::CollatorSelection(Event::StakeRemoved {
			staker: 5,
			candidate: 3,
			amount: 20,
		}));
		System::assert_has_event(RuntimeEvent::CollatorSelection(Event::UnstakeRequestCreated {
			staker: 5,
			amount: 20,
			block: 3,
		}));
...
fn unstake_self() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(StakeCount::<Test>::get(3), 1);
		assert_ok!(CollatorSelection::unstake_from(RuntimeOrigin::signed(3), 3));
		System::assert_last_event(RuntimeEvent::CollatorSelection(Event::StakeRemoved {
			staker: 3,
			candidate: 3,
			amount: 30,
...
fn unstake_fails_if_over_limit() {
	new_test_ext().execute_with(|| {
		...
		for pos in 3..=18 {
			assert_ok!(CollatorSelection::stake(RuntimeOrigin::signed(5), pos, 2));
		}
		// now we accumulate 16 requests, the maximum.
		assert_ok!(CollatorSelection::unstake_all(RuntimeOrigin::signed(5)));
		assert_eq!(UnstakingRequests::<Test>::get(5).len(), 16);

		assert_ok!(CollatorSelection::stake(RuntimeOrigin::signed(5), 3, 2));
		assert_noop!(
			CollatorSelection::unstake_from(RuntimeOrigin::signed(5), 3),
			Error::<Test>::TooManyUnstakingRequests
		);

		// if we claim the requests we can keep unstaking.
		initialize_to_block(3);
		assert_ok!(CollatorSelection::claim(RuntimeOrigin::signed(5)));
		assert_ok!(CollatorSelection::unstake_from(RuntimeOrigin::signed(5), 3));
	});
}
...
fn claim() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(CollatorSelection::claim(RuntimeOrigin::signed(5)));
		assert_eq!(UnstakingRequests::<Test>::get(5), vec![]);
		System::assert_last_event(RuntimeEvent::CollatorSelection(Event::StakeClaimed {
			staker: 5,
			amount: 20,
		}));
	});
...
fn set_collator_reward_percentage() {
	new_test_ext().execute_with(|| {
		...
		// Invalid origin
		assert_noop!(
			CollatorSelection::set_collator_reward_percentage(
				RuntimeOrigin::signed(5),
				Percent::from_parts(50)
			),
			BadOrigin
		);
		assert_ok!(CollatorSelection::set_collator_reward_percentage(
			RuntimeOrigin::signed(RootAccount::get()),
			Percent::from_parts(50)
		));
		System::assert_last_event(RuntimeEvent::CollatorSelection(
...
fn set_minimum_stake() {
	new_test_ext().execute_with(|| {
		...
		// Invalid origin
		assert_noop!(CollatorSelection::set_minimum_stake(RuntimeOrigin::signed(5), 5), BadOrigin);

		// Set the reward over CandidacyBond
		assert_noop!(
			CollatorSelection::set_minimum_stake(RuntimeOrigin::signed(RootAccount::get()), 1000),
			Error::<Test>::InvalidMinStake
		);

		// Zero is a valid value
		assert_ok!(CollatorSelection::set_minimum_stake(
			RuntimeOrigin::signed(RootAccount::get()),
			0
		));
		System::assert_last_event(RuntimeEvent::CollatorSelection(Event::NewMinStake {
			min_stake: 0,
		}));
...
fn should_not_reward_invulnerables() {
	new_test_ext().execute_with(|| {
		...
		assert!(!System::events().iter().any(|e| {
			match e.event {
				RuntimeEvent::CollatorSelection(Event::StakingRewardReceived { .. }) => true,
				_ => false,
			}
		}));
	});
...
fn should_reward_collator() {
	new_test_ext().execute_with(|| {
		...
		// Reward for collator
		System::assert_has_event(RuntimeEvent::CollatorSelection(Event::StakingRewardReceived {
			staker: 4,
			amount: 1,
		}));
		// Reward for staker
...
fn should_reward_collator_with_extra_rewards_and_many_stakers() {
	new_test_ext().execute_with(|| {
		assert_ok!(CollatorSelection::register_as_candidate(RuntimeOrigin::signed(3),));
		assert_ok!(CollatorSelection::register_as_candidate(RuntimeOrigin::signed(4),));
		assert_ok!(CollatorSelection::stake(RuntimeOrigin::signed(2), 4, 40));
		assert_ok!(CollatorSelection::stake(RuntimeOrigin::signed(3), 4, 50));
